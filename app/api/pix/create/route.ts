...
export async function POST(request: NextRequest) {
  try {
    ...
    const blackCatRequest: BlackCatCreateSaleRequest = {
      ...
      postbackUrl: `${process.env.NEXT_PUBLIC_APP_URL || "https://papelariaestelar.netlify.app"}/api/webhook/blackcat`,
      externalRef: orderId,
    ...
    // CORREÇÃO: Tenta capturar o QR Code de múltiplos campos possíveis da API Black Cat
    const qrCode = paymentData.copyPaste || paymentData.qrCode || (paymentData as any).pix_code || (paymentData as any).emv;
    ...
    try {
      ...
      console.error("[PIX Create] Erro UTMify:", utmfyError)
    }
    ...
    return NextResponse.json({
      ...
      transactionId,
      pix: {
        qrcode: qrCode, // Retorna o código capturado
